# AGENTS.md — wdlib-code 代理协作规范

本文件为 codex 等编码代理在 `wdlib-code/` 目录内进行代码生成、修改、重构时必须遵守的规范。完整背景与不一致审计见 [STYLE.md](STYLE.md)。
...
- **语言**：中文。
- **头文件不写注释**：`.h` 文件中**禁止**出现解释性注释（模块简介、函数说明、字段说明等）。唯一允许的是区域标记：固定使用 `// 依赖`、`// 配置`、`// 类型定义`、`// 接口` 分隔（**禁止**使用 `// 类定义`）。所有公开接口的文档统一写在 `wdlib-doc` 中。
- **源文件算法注释**：非显而易见的算法或设计决策用 `/* */` 块注释解释**为什么**，不解释**是什么**。
- **源文件关键步骤注释**：`.c` 文件中的关键步骤必须添加简短注释，说明该步骤的意图。关键步骤包括但不限于：状态切换、资源获取/释放、重要条件分支、数据结构关键操作（如链表插入/删除、索引回绕）。注释用 `//` 行注释，放在对应代码行上方或行尾，保持简洁。
- **不写注释的情况**：
  - 函数名/变量名已经足够说明意图；
  - 解释代码做了什么（代码自身应可读）；
  - 引用 issue/PR 号或调用者。
...
- [ ] 头文件中**无解释性注释**（仅保留区域标记），文档写在 `wdlib-doc` 中。
...
- [ ] 整型字面量带 `U` 后缀。
- [ ] `.c` 文件中关键步骤（状态切换、资源管理、重要分支、数据结构关键操作）已添加简短注释。
